...
class SchoolDistrictView(BaseModelView):
    extra_js = ["https://app.simplefileupload.com/buckets/940266945f5018c8b382023c3251749d.js"]
    ...
    # Need to handle editing of this with Simple File Upload html
    # form_excluded_columns = ["logo_url"]

    column_formatters = {"logo_url": _render_img}

    def on_model_change(self, form, model, is_created):
        print("Logo upload output: ", model.logo_url)
...
class ManageDataView(BaseView):
    ...
    def sync(cls):
        ...
        table = api.table('appYtpMh7yl7Sp6qu', 'tblshGUR0qqyZF3px')
        districts = table.all()
        for district in districts:
            ...
            existing_district =  SchoolDistrict.query.filter_by(nces_id=nces_id).filter_by(name=name).first()
            ...
            existing_district.superintendent_name = district["fields"].get("Superintendent-Name")
            existing_district.superintendent_email = district["fields"].get("Superintendent-Email")
            existing_district.civil_rights_url = district["fields"].get("CivilRights-URL")
            existing_district.civil_rights_contact_name = district["fields"].get("CivilRights-Contact")
            existing_district.civil_rights_contact_email = district["fields"].get("CivilRights-Email")
    ...
    def process_csv(self, file_io):
        ...
        for row in csv_reader:
            ...
            existing_district = SchoolDistrict.query.filter_by(nces_id=nces_id).filter_by(name=district_name).first()
